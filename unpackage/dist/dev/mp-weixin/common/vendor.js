...
module.exports = {
  ...
  /**
      *设置数据存储
      * */
  "setStorageInfo": function setStorageInfo(key, data, callback) {
    uni.setStorage({
      key: key,
      data: data,
  ...
  /** 
      * 获取存储数据
      */
  "getStorageInfo": function getStorageInfo(key, callback) {
    uni.getStorage({
      key: key,
      success: function success(result) {
        if (callback) {
  ...
  /** 
      * 验证AccessToken是否过期
      */
  "validateAccessToken": function validateAccessToken(key, callback) {
    //无身份信息
  ...
  /** 
      * 无回调验证AccessToken过期
      */
  "validateAccessTokenSync": function validateAccessTokenSync(key) {
    if (isEmpty(uni.getStorageSync(key))) {
      return false;
    } else {
      //token的过期日期 大于 现在
  ...
  /**
      * 提交请求
      */
  "Request": function Request(url, method, data, _success, _fail) {
  ...
  /**
      * 小写数字转大写汉字
      */
  "smalltoBIG": function smalltoBIG(value) {
  ...
  /**
      * 计算两个时间的时间差
      */
  "calcuDateLong": function calcuDateLong(sdate, edate, callback) {
    if (!sdate) {
      uni.showToast({
        title: '开始时间为空',
        icon: 'none' });
  ...
  /**
      * 日期、时间格式化
      */
  "formatDateTime": function formatDateTime(date, fmt) {//author: meizz   
    var d = new Date(date);
    var o = {
      "M+": d.getMonth() + 1, //月份   
      "d+": d.getDate(), //日   
  ...
  /**
      * 生成单据编码
      */
  "getEntityNum": function getEntityNum() {
    var d = new Date();
  ...
  /**
      * 列表页判断是否刷新
      */
  "getIsRefresh": function getIsRefresh() {
  ...
  /**
      *设置列表页控制刷新  
      **/
  "setIsRefresh": function setIsRefresh(val) {
...
{
  hxmd5: hex_md5 };exports.default = _default;
