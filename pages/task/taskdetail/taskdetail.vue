	...
	<view>
		...
		<view>
			<view class="cu-bar bg-white solid-bottom solid-top">
				...
				<view class="action">
					<button class='cu-btn bg-gray icon' @click="showhide('order',$event)"><text :class="[isOrder?'icon-fold':'icon-unfold']"></text></button>
				</view>
			</view>
			<view class="cu-list menu sm-border" v-if="isOrder">
				<view class="bg-white padding">
					<view class="cu-form-group">
						<text maxlength="-1" disabled="disabled">{{this.$mbservices.isEmpty(appNote.RequestContent)? '暂无单据信息' : appNote.RequestContent}}</text>
					</view>
				</view>
			</view>
		</view>
		...
		<view>
			<view class="cu-bar bg-white solid-bottom solid-top">
				...
				<view class="action">
					<button class='cu-btn bg-gray icon' @click="showhide('flow',$event)"><text :class="[isFlow?'icon-fold':'icon-unfold']"></text></button>
				</view>
			</view>
			<view class="cu-list menu sm-border" v-if="isFlow">

				<view class="bg-white padding">
					<view class="cu-steps">
						<view class="cu-item" :class="item.ApproveStatus=='Approved'?'text-green':item.ApproveStatus=='Rejected'?'text-red':item.ApproveStatus=='Pending'?'text-blue':''"
						 v-for="(item,index) in appStages" :key="index">
			...
			<view class="bg-white padding">
				<view class="cu-form-group">
					<textarea data-placeholder="在此输入审批意见" maxlength="-1" :disabled="modalName!=null" placeholder-class='placeholder'
					 :class="textareaAValue?'value':''" @input="textareaAInput" :value="textareaAValue"></textarea>
				</view>
			</view>
			<view class="cu-bar bg-white solid-bottom solid-top">
				<view class="action">
