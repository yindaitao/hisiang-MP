...
<script>
	export default {
		...
		methods: {
			goDetail(item) {
				uni.navigateTo({
					url: "/pages/ReimbursementRequest/detail/cadetail?data=" + JSON.stringify(item)
				});
			},
			editItem(item) {
				console.log(item);
				uni.navigateTo({
					url: "/pages/ReimbursementRequest/form/form?flag=modify&data=" + JSON.stringify(item)
				});
			},
			deleteItem(item) {
				this.modalName = null;
			...
			addWorkOrder() {
				uni.navigateTo({
					url: "/pages/ReimbursementRequest/form/form"
				});
			},
