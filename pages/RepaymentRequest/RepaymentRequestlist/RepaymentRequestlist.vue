					 ...
					 @tap="goDetail(list)" style="position: relative;">
						...
						<view class="content padding-tb-sm">
							...
							<view>
								...
								单号:{{list.DocEntry}}&nbsp;&nbsp;还款金额:{{list.Amount}}
							</view>
		...
		methods: {
			goDetail(item) {
				if(item.Approve==='No'&&item.ApproveStatus!=='Rejected'){
					...
					uni.navigateTo({
						url: "/pages/RepaymentRequest/RepaymentRequestform/RepaymentRequestform?flag=Original&data=" + JSON.stringify(item)
					});
				}
			},
			...
			addWorkOrder() {
				uni.navigateTo({
					url: "/pages/RepaymentRequest/RepaymentRequestform/RepaymentRequestform"
				});
			},
			makeParams() {
