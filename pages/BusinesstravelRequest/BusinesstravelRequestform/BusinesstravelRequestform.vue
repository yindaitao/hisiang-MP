<template>
	<view>
		...
		<view id="_tabBar" class="cu-bar bg-white solid-bottom top" style="position: fixed;display: flex;z-index: 2;z-index: 999;width: 100%;">
			<view class="action">
				<text class="icon-redpacket text-orange"></text>
				总金额:{{totalJine}}(元)
			</view>
			<view class="action" v-if='edit === false'>
				<button class="cu-btn round bg-blue shadow sm" data-target="DialogModal2" @tap="showModal">
					<text class="icon-upload"></text><text class="margin-left-sm">提交</text>
				</button>
			</view>
		</view>
		...
		<view class="ul-swiper-box margin-top" style="margin-top: 50px;">
			<form>
				...
				<view class="cu-form-group">
					...
					<input :disabled="edit?true:false" placeholder="出差天数" name="input" style="text-align: right;" @input="inputNumDays($event)"
					 :value="itemData.Days">
					<text v-if="false" class="icon-roundclosefill text-orange"></text>
				</view>
				<view class="cu-form-group">
				...
				<view class="cu-form-group">
					<textarea :disabled="modalName!=null" @input="textareaInputReasons" :class="itemData.Reasons?'value':''" maxlength="-1"
					 placeholder-class="placeholder" data-placeholder="在此输入出差事由" :value="itemData.Reasons"></textarea>
				</view>
				<block v-for="(item,index) in formList" :key="index">
					...
					<view class="cu-form-group">
						...
						<input :disabled="edit?true:false" placeholder="请输入报销金额" name="input" type="digit" style="text-align: right;"
						 @input="inputNum(item,$event)" :value="item.jine">
						<text v-if="false" class="icon-roundclosefill text-orange"></text>
					</view>
					<view class="cu-form-group">
					...
					<view class="cu-form-group">
						...
						<input :disabled="edit?true:false" placeholder="单据张数" name="input" style="text-align: right;" @input="inputNumCount1(item,$event)"
						 :value="item.Count1">
						<text v-if="false" class="icon-roundclosefill text-orange"></text>
					</view>
					<view class="cu-form-group">
					...
					<view class="cu-form-group" v-if="item.DetailType === 'Traffic'">
						...
						<picker :disabled="edit?true:false" mode="date" :value="item.DocDateStart" :start="startDate" :end="endDate"
						 @change="bindDateStartChange(item,$event)">
							<view class="picker">{{item.DocDateStart}}</view>
						</picker>
					</view>
					<view class="cu-form-group" v-if="item.DetailType === 'Traffic'">
						<view class="title">出发地点</view>
						<input :disabled="edit?true:false" placeholder="出发地点" name="input" style="text-align: right;" @input="inputNumStartPlace(item,$event)"
						 :value="item.StartPlace">
						<text v-if="false" class="icon-roundclosefill text-orange"></text>
					</view>
					<view class="cu-form-group" v-if="item.DetailType === 'Traffic'">
						<view class="title">到达日期</view>
						<picker :disabled="edit?true:false" mode="date" :value="item.DocDateArrive" :start="startDate" :end="endDate"
						 @change="bindDateArriveChange(item,$event)">
					...
					<view class="cu-form-group">
						<textarea @input="textareaInput(item,$event)" :class="item.Remarks1?'value':''" maxlength="-1" placeholder-class="placeholder"
						 data-placeholder="在此输入明细备注" :value="item.Remarks1"></textarea>
					</view>
					<!-- 图片开始 -->
					<view class="cu-bar bg-white">
					...
					<view class="cu-form-group">
						<view class="grid col-4 grid-square flex-sub">
							<view class="padding-xs bg-img" :style="'background-image:url(' + item.imageList[index1].url +')'" v-for="(_item,index1) in item.imageList"
							 :key="index1" @tap="previewImage(item,$event)" :data-url="item.imageList[index1].url">
				...
				<uni-view class="cu-bar bg-gray solid-bottom" style="width: 100%;" v-if="edit === false">
					<button class="cu-btn round bg-blue shadow sm" style="margin: 0 auto;" @tap="addOption">
						<text class="icon-add"></text><text class="margin-left-sm">增加&nbsp;&nbsp;差旅报销明细</text>
					</button>
				</uni-view>
				<view class="cu-form-group" readonly>
				...
				<view class="cu-form-group">
					<textarea @input="textareaInput33" :class="itemData.Remarks?'value':''" maxlength="-1" :disabled="modalName!=null"
					 id="_Remarks" name="_Remarks" placeholder-class="placeholder" data-placeholder="在此输入备注" :value="itemData.Remarks"></textarea>
				</view>
			</form>
		</view>
		<!-- </scroll-view> -->
...
<script>
	...
	export default {
		...
		data() {
			return {
				scrollBarHeight: 0,
				PayType: ["转账给申请人", "转账给第三人(需备注)", "银行转账(需备注)", "现金支付给申请人", "冲借款", "按发票汇款", "银行托收", "申请支票",
					"其他现金支付", "其他银行汇款", "无需支付"
				],
				indexPayType: 0,
				indexCostType: 0,
				...
				ReimbursementNameList: ["私人费用", "分摊费用", "部门费用", "内部往来费用", "工会会费"],
				...
				formList: [{
					...
					name: "张三",
					jine: "",
					DocDateStart: this.getDate({
						format: true
					}),
					DocDateArrive: this.getDate({
						format: true
					}),
					StartPlace: "",
					ArrivePlace: "",
