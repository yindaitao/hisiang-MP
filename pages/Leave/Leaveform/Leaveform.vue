		...
		methods: {
			...
			CalcDateTime() {
				...
				switch (this.itemData.LeaveHoursText) {
					case 'Hour':
						if (!this.$mbservices.isEmpty(this.itemData.EndDate)) {
							this.itemData.LeaveHours = parseFloat((new Date(this.itemData.EndDate.toString().replace(/-/g, '/')).valueOf() -
								new Date(
									this.itemData.BeginDate.toString().replace(/-/g, '/')).valueOf()) / 3600000).toFixed(2);
						}
						if (this.itemData.LeaveHours <= 0) {
							uni.showToast({
								title: '结束时间小于或等于开始时间',
								icon: 'none'
							})
							this.$refs.PEndDate1.refresh()
							this.itemData.LeaveHours = "";
							return false;
						}
						if (this.itemData.LeaveHours >= 8) {
							uni.showToast({
								title: '按小时请假不得大于8小时',
								icon: 'none'
							})
					...
					case 'Day':
						if (!this.$mbservices.isEmpty(this.itemData.EndDate)) {
							...
							this.itemData.LeaveHours = ccc;
							if (this.itemData.LeaveHours <= 0) {
								this.itemData.LeaveHours = 1.00;
							}
						}
						break;
					default:
			...
			showModal(e) {
				if (this.$mbservices.isEmpty(this.itemData.LeaveHours) || this.$mbservices.isEmpty(this.itemData.BeginDate) || this
					.$mbservices.isEmpty(this.itemData.EndDate) ||
				...
				//return false;
				if (!this.checkbox[0].checked && (this.$mbservices.isEmpty(this.itemData.HolidayTypeCode) || this.itemData.HolidayTypeCode ===
						'IsChangeShifts')) {
					uni.showModal({
						title: "提示",
						content: "请选择请假类型",
			...
			computTime() {
				if (this.$mbservices.isEmpty(this.resultInfo1.result) || this.$mbservices.isEmpty(this.resultInfo2.result)) {
					return false;
				}
						...
						for (var i in this.HolidayScheduleList) {
							if (this.HolidayScheduleList[i].Date === date) {
								...
								if (hour1 <= beginHour) {
									...
									if (hour2 < endHour) {
										this.itemData.LeaveHours = (everyDay).toFixed(2) - ((time1 - Btime) / 1000 / 3600).toFixed(2) - ((Etime -
											time2) / 1000 / 3600).toFixed(2);
									} else if (hour2 >= endHour) {
