<template>
    <view>
        ...
        <scroll-view
            ...
            ref="_tabBar"
            scroll-x
            class="cu-bar bg-white nav text-center"
        >
            <button
                class="cu-btn icon bg-gray"
                style="position: absolute;left: 2px;"
                @tap="showModalSearch"
                data-target="DrawerModalR"
            ><text class="icon-filter"><span></span></text></button>
            <view
                class="cu-item"
                :class="index==TabCur?'text-blue cur':''"
                v-for="(item,index) in tabBars"
                :key="index"
                @tap="tapTab(index,$event)"
                :data-current="index"
                :data-id="index"
                :id="'_tabBarItem'+item.id"
            >
                <text>
                    {{item.name}}
                </text>
            ...
            <button
                ...
                @tap="doRefresh"
                data-target="DrawerModalR"
            ><text class="icon-refresh"><span></span></text></button>
        </scroll-view>
        <swiper
            :current="TabCur"
            class="swiper-box"
                        ...
                        >
                            <view class="content padding-tb-sm">
                                <view>
                                    ...
                                    审批编号:<text class="text-pink">{{list.DocEntry}}</text>&nbsp;&nbsp;&nbsp;<label style="margin-left: 15px;">单据编号:</label><text class="text-brown">{{list.BusinessOrderNo}}</text>
                                </view>
                                <view>
                                    ...
                                    >{{list.OriginatorName}}</text>
                                </view>
                                <view>
                                    <text class="icon-activityfill text-blue margin-right-xs"></text>
                                    申请类型:
                                    <text
                                        v-if="list.BusinessType==='BorrowRequest'"
                                        class="bg-gradual-green radius text-center shadow-blur padding-lr margin-left-sm text-ABC"
                                        style="min-width: 100px;"
                                    >{{list.BBusinessType}}</text>
                                    <text
                                        v-else-if="list.BusinessType==='BusinesstravelRequest'"
                                        class="bg-gradual-orange radius text-center shadow-blur padding-lr margin-left-sm text-ABC"
                                        style="min-width: 100px;"
                                    >{{list.BBusinessType}}</text>
                                    <text
                                        v-else-if="list.BusinessType==='ReimbursementRequest'"
                                        class="bg-gradual-purple radius text-center shadow-blur padding-lr margin-left-sm text-ABC"
                                        style="min-width: 100px;"
                                    >{{list.BBusinessType}}</text>
                                    <text
                                        v-else-if="list.BusinessType==='RepaymentRequest'"
                                        class="bg-gradual-blue radius text-center shadow-blur padding-lr margin-left-sm text-ABC"
                                        style="min-width: 100px;"
                                    >{{list.BBusinessType}}</text>
                                    <text
                                        v-else-if="list.BusinessType==='AdvancePayRequest'"
                                        class="bg-gradual-red radius text-center shadow-blur padding-lr margin-left-sm text-ABC"
                                        style="min-width: 100px;"
                                    >{{list.BBusinessType}}</text>
                                    <text
                                        v-else
                            ...
                            <view class="action">
                                <view
                                    v-if="list.ApproveStatus==='Pending'"
                                    class="cu-tag round bg-olive light"
                                >{{list.AApproveStatus}}</view>
                                <view
                                    v-if="list.ApproveStatus==='Approved'"
                                    class="cu-tag round bg-blue light"
                                >{{list.AApproveStatus}}</view>
                                <view
                                    v-if="list.ApproveStatus==='Rejected'"
        ...
        <view
            ...
            :class="modalNameSearch=='DrawerModalR'?'show':''"
            @tap="hideModalSearch"
        >
            <view
            ...
            >
                <view class="cu-bar bg-gray">
                    <view class="action">
                        ...
                        >取消</view>
                        <view
                            class="text-bold margin-left-sm"
                            @tap="resetConditions"
                        >重置</view>
                    </view>
                    <view
                        class="action text-green"
                        @tap="doSearchApprovalList"
                    >确定</view>
                </view>
                <view class="box">
