		...
		<scroll-view scroll-y class="page" :style="{'height':scrollBarHeight+'px'}">
			<view class="bg-white">
				<view class="content solid-bottom">
					<model-calendar :sendYear="toYear" :sendMonth="toMonth" :dataSource="signData"
					 :totalNum="sumCount" @dateChange="getData" @clickChange="clickSign">
					</model-calendar>
				</view>
...
<script>
	...
	/* import tableCalendar from '@/components/table-calendar/table-calendar.vue'; */
	export default {
		data() {
		...
		methods: {
			...
			getWorkRecords(e, from) {
				...
				/* if (this.HolidayScheduleList === null || this.HolidayScheduleList.data.RecordCount <= 0) {
					uni.showToast({
						title: '获取公休时间表异常,请联系管理员',
						icon: 'none'
					})
					return false;
				} */
				/* var isGongXiu = false;
				this.HolidayScheduleList.data.data.forEach((itm, idx) => {
					if (this.$mbservices.formatDateTime(itm.Date, 'yyyy-MM-dd') === this.$mbservices.formatDateTime(e, 'yyyy-MM-dd')) {
						isGongXiu = true;
					}
				})
				if (isGongXiu) {
